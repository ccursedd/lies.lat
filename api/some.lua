...
local clone_char = function(model)
	...
	local forcefield = new_clone:FindFirstChildWhichIsA("ForceField")
	if forcefield then
		forcefield:Destroy();
	end;
	return new_clone;
...
-- New function to parse animation data without loadstring
local parse_animation_data = function(response)
	-- Try JSON first (safer)
	local success, data = pcall(function()
		return mevreanim.services.http_service:JSONDecode(response)
	end)
	...
	-- If not JSON, try to safely evaluate Lua table
	local func, err = load("return " .. response, "animation_data", "t", {})
	if not func then
		return nil, "Failed to parse animation data: " .. tostring(err)
	end
	
	local success2, data2 = pcall(func)
	if not success2 then
		return nil, "Failed to execute animation data: " .. tostring(data2)
	end
	
	if type(data2) ~= "table" then
		return nil, "Animation data must be a table"
	end
...
API.stop_animation = function()
	...
	clearTable(anim.pending_c0s);
	clearTable(anim.original_motor_c0s);
	clearTable(anim.joints);
	anim.animator = nil;
	anim.state = {
		is_playing = false;
		current_url = nil;
...
API.play_animation = function(url, speed)
	...
	if not keyframe_data then
		...
		local data, err = parse_animation_data(response);
		if not data then return "Animation Error: " .. err end;
		
		keyframe_data = data;
		anim.cache[url] = keyframe_data;
	end;

	-- Handle both table formats (with named keys or direct array)
	local keyframes = keyframe_data
	if keyframe_data.keyframes then
		keyframes = keyframe_data.keyframes
	elseif keyframe_data[next(keyframe_data)] and type(keyframe_data[next(keyframe_data)]) == "table" then
	...
	mevreanim.connections.animation_hb = mevreanim.services.run_service.Heartbeat:Connect(function(dt)
		...
		anim.state.elapsed_time = anim.state.elapsed_time + dt * anim.state.speed;
		if anim.state.elapsed_time >= anim.state.total_duration then
			-- Loop or stop? Let's loop by default
